...
Data is processed by 3 LLMs + a sampler.
    ...
    - Judge: Evaluates the Generator results based on 3 criteria - realism, consistency, and completeness. If it passes, it's saved; if it fails, then it makes a critique on why it failed (text value). This fail condition is a loop with the refiner LLM.

    - Refiner: In a loop with the judge LLM. Results that the judge fails go here. It looks at the critique and human samples it's based on, and edits the offending column value. It outputs an updated JSON row that goes back to the judge. This is capped at 2 retries and then thrown away.
...
- nodes: Each LLM gets a function which is a node.
    - sampler_node: Updates grounding_seeds
    - generator_node: Uses grounding_seeds to call generator, updates generated_row
...
- LangGraph: A subset of LangChain. It's made for state machines and complex non-linear workloads. You can add, complete, and summarize tasks. Each of these is 1 node (graph structure) with edges between them. These are fed by a process input (which is NOT an LLM) that routes to the correct node. There's also a state component that tracks each state. The idea is you keep context over long-term interactions.
    ...
    - state is a dictionary and is the shared memory passed between functions.
